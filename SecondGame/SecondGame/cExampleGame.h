...
namespace eae6320
{
	...
	private:
		...
		float upBoundary_ = 0.6f, downBoundary_ = -0.7f, leftBoundary_ = -0.6f, rightBoundary_ = 0.6f;

		bool firstSpawnEnemy_;
		float timer_;
		float nextSpawnInterval_;
		float initInterval_ = 1.0f;
		float spawnMinInterval_ = 1.5f;
		float spawnMaxInterval_ = 2.5f;
		int killNumber_;
		int goalKillNumber_ = 15;

		float playerSpeed_ = 0.6f;
		float bulletExtendX_ = 0.005f;
		float bulletExtendY_ = 0.01f;
		float enemyExtend_ = 0.03f;
		float playerExtend_ = 0.04f;
	};
}
