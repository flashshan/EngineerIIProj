...
eae6320::cResult eae6320::Graphics::cMesh::Initialize(const VertexFormats::sGeometry *i_vertexData, unsigned int i_vertexCount, const uint16_t *i_indexData, unsigned int i_indexCount)
{
	...
	// Create a vertex array object and make it active for vertex
	{
	...
	// Assign the data to the buffer
	{
		...
		glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(bufferSize), reinterpret_cast<const GLvoid*>(i_vertexData),
	...
	}
	{
		...
		EAE6320_ASSERT(bufferSize < (uint64_t(1u) << (sizeof(GLsizeiptr) * 8)));
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizeiptr>(bufferSize), reinterpret_cast<const GLvoid*>(i_indexData),
			// In our class we won't ever read from the buffer
			GL_STATIC_DRAW);
		const auto errorCode = glGetError();
		if (errorCode != GL_NO_ERROR)
	...
	// Initialize vertex format
	{
		...
		}
		{
			...
			glVertexAttribPointer(vertexElementLocation, elementCount, GL_UNSIGNED_BYTE, notNormalized, stride,
				reinterpret_cast<GLvoid*>(offsetof(eae6320::Graphics::VertexFormats::sGeometry, r)));
			const auto errorCode = glGetError();
			if (errorCode == GL_NO_ERROR)
			{
				glEnableVertexAttribArray(vertexElementLocation);
...
void eae6320::Graphics::cMesh::Draw()
{
	// Draw the geometry
	{
		...
		// Render triangles from the currently-bound vertex buffer
		{
			...
			glDrawElements(mode, static_cast<GLsizei>(indexCount_), GL_UNSIGNED_SHORT, offset);
			EAE6320_ASSERT(glGetError() == GL_NO_ERROR);
		}
	}
}
