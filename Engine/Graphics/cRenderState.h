...
namespace eae6320
{
	namespace Graphics
	{
		namespace RenderStates
		{
			...
			// Alpha Transparency
			inline bool IsAlphaTransparencyEnabled( const uint8_t i_renderStateBits );
			inline void EnableAlphaTransparency( uint8_t& io_renderStateBits );
			inline void DisableAlphaTransparency( uint8_t& io_renderStateBits );
			// Depth Buffering
			inline bool IsDepthBufferingEnabled( const uint8_t i_renderStateBits );
			inline void EnableDepthBuffering( uint8_t& io_renderStateBits );
			inline void DisableDepthBuffering( uint8_t& io_renderStateBits );
			// Draw Both Triangle Sides
			inline bool ShouldBothTriangleSidesBeDrawn( const uint8_t i_renderStateBits );
			inline void EnableDrawingBothTriangleSides( uint8_t& io_renderStateBits );
			inline void DisableDrawingBothTriangleSides( uint8_t& io_renderStateBits );
		}
	}
}
...
namespace eae6320
{
	namespace Graphics
	{
		...
		public:
			...
			inline bool IsAlphaTransparencyEnabled() const;
			inline bool IsDepthBufferingEnabled() const;
			inline bool ShouldBothTriangleSidesBeDrawn() const;
