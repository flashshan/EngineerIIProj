...
void eae6320::Application::cbApplication::CalcRectangleVertexDataFromUserInput(EAlignmentXType i_alignmentXType, EAlignmentYType i_alignmentYType,
	float i_offsetX, float i_offsetY, float i_scaleX, float i_scaleY, Graphics::VertexFormats::sSprite *o_vertexData)
{
	float baseX = i_offsetX * 2.0f / float(m_resolutionWidth) * float(m_resolutionHeight), baseY = i_offsetY * 2.0f;
	switch (i_alignmentXType)
	{
	case EAlignmentXType::ELeft:
		baseX += -1.0f;
		break;
	case EAlignmentXType::ECenter:
		baseX += 0.0f;
		break;
	case EAlignmentXType::ERight:
		baseX += 1.0f;
		break;
	}

	switch (i_alignmentYType)
	{
	case EAlignmentYType::EBottom:
		baseY += -1.0f;
		break;
	case EAlignmentYType::EMiddle:
		baseY += 0.0f;
		break;
	case EAlignmentYType::ETop:
		baseY += 1.0f;
		break;
	}

	float endX = baseX + i_scaleX * 2.0f / float(m_resolutionWidth) * float(m_resolutionHeight), endY = baseY + i_scaleY * 2.0f;
	o_vertexData[0].x = baseX; o_vertexData[0].y = baseY;
	o_vertexData[0].u = 0.0f; o_vertexData[0].v = 0.0f;

	o_vertexData[1].x = endX; o_vertexData[1].y = baseY;
	o_vertexData[1].u = 1.0f; o_vertexData[1].v = 0.0f;

	o_vertexData[2].x = baseX; o_vertexData[2].y = endY;
	o_vertexData[2].u = 0.0f; o_vertexData[2].v = 1.0f;

	o_vertexData[3].x = endX; o_vertexData[3].y = endY;
	o_vertexData[3].u = 1.0f; o_vertexData[3].v = 1.0f;
}
	...
	while ( !m_shouldApplicationLoopExit )
	{
		...
		// after it has finished rendering the current frame with the previously-submitted data
		{
			...
			// Submit the data to be rendered
			{
				...
				{
					SubmitDataToBeRendered( elapsedSecondCount_systemTime, static_cast<float>( Time::ConvertTicksToSeconds( tickCount_simulationTime_elapsedButNotYetSimulated ) ) );
				}
