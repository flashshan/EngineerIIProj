...
{
	float2 uv = float2(i_uv.x + g_elapsedSecondCount_simulationTime, i_uv.y);

	const float4 sampledColor = GetSampleColor(uv);

	o_color = float4(1.0, 1.0, 1.0, 0.5 + cos(g_elapsedSecondCount_simulationTime) * 0.5) * sampledColor;
